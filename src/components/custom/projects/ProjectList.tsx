	...
	const filterProjects = (
		...
		return projects.filter((project) => {
			...
			if (filters.technologies.length > 0) {
				const hasMatchingTechnology = filters.technologies.some(
					(filterTech) => project.technologies.includes(filterTech)
				);
			...
			if (filters.dateRange.start || filters.dateRange.end) {
				...
				if (
					...
					projectEnd < filters.dateRange.start
				) {
					return false;
				}
				...
				if (
					...
					projectStart > filters.dateRange.end
				) {
					return false;
				}
			}
			...
			<div className="mb-6 flex justify-between items-center">
				...
				{filteredProjects.length !== projects.length && (
					<p className="text-sm text-blue-600">
						{projects.length - filteredProjects.length} projects
						filtered out
					</p>
				)}
			</div>
